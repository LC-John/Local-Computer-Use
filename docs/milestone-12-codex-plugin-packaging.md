...
Status: Repo-local plugin bundle implemented and validated. Codex host loading
and new-thread smoke testing remain manual follow-up because this repository is
not currently installed through a Codex marketplace entry.
...
This milestone creates a repo-local plugin bundle. To load it in Codex, install
or reference this repository as a local plugin source using Codex's local plugin
workflow. Do not overwrite the bundled OpenAI `computer-use` plugin.
...
- Install the plugin into a Codex host environment.
